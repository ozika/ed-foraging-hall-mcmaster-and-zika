...
# data set 
# "" = stadnard
# "_alt_cutoff" = alternative cutoff
...
for (prop in c(0.7, 0.8, 0.9)) {
  for (rid in seq(500)) {
    print(rid)
    n_sample <- ceiling(length(unique_ids) * prop)
    sampled_items <- sample(unique_ids, n_sample)

    dtl <- data[data$id %in% sampled_items,]
    
    slmod <- glmmTMB::glmmTMB(data=dtl, RT ~ decay_fac*travel_time_lvl*group*remaining_time_reversed + (1+decay_fac+travel_time_lvl|id), family = Gamma(link=linkfunc))
    
    an<-car::Anova(slmod)
    
    
    
    em <- emtrends(slmod, pairwise ~  group , var = "remaining_time_reversed", adjust=adjustment_method)
    em
    
    tt <- data.frame(em$contrasts)
    tt$prop <- prop
    tt$runid <- rid
    
    rundf <- cbind(data.frame(an)["group:remaining_time_reversed",], tt)
    expdf <- rbind(expdf, rundf)
  }
  
}

write.csv(expdf, here::here( 'output', 'group_rem_time_bootstrap_500.csv'))
...
```{r}
#write.csv(expdf, here::here( 'output', 'group_rem_time_bootstrap_500.csv'))
expdf = read.csv(here::here( 'output', 'group_rem_time_bootstrap_500.csv'))
library(ggplot2)
library(dplyr)

# Calculate percentiles
p_low <- quantile(expdf$Pr..Chisq., 0.025)
p_high <- quantile(expdf$Pr..Chisq., 0.975)

ggplot(expdf, aes(x = Pr..Chisq.)) +
  geom_histogram(
    bins = 100,
    aes(fill = ..x.. >= p_low & ..x.. <= p_high),
    color = "black"
  ) +
  scale_fill_manual(values = c("FALSE" = "dodgerblue", "TRUE" = "pink"), guide = "none") +
  
  # Vertical lines with limited y range using geom_segment
  #geom_segment(aes(x = p_low, xend = p_low, y = 0, yend = 80), color = "red", linetype = "dashed") +
  #geom_segment(aes(x = p_high, xend = p_high, y = 0, yend = 80), color = "red", linetype = "dashed") +
  #geom_segment(aes(x = 49.5, xend = 49.5, y = 0, yend = 85), color = "darkgreen", linetype = "dashed") +
  
  # Labels at top of lines
  #annotate("text", x = p_low, y = 82, 
  #         label = paste0("X²(2.5%) = ", round(p_low, 2)), 
  #         vjust = -0.5, color = "red") +
  #annotate("text", x = p_high, y = 82, 
  #         label = paste0("X²(97.5%) = ", round(p_high, 2)), 
  #         vjust = -0.5, color = "red") +
  #annotate("text", x = 55, y = 90, 
  #         label = "Full sample statistic \n(X² = 49.5)", 
...
# Calculate percentiles
p_low <- quantile(expdf$Chisq, 0.025)
p_high <- quantile(expdf$Chisq, 0.975)

gg<-ggplot(expdf, aes(x = Chisq)) +
  geom_histogram(
    bins = 50,
    aes(fill = ..x.. >= p_low & ..x.. <= p_high),
    color = "black"
  ...
  geom_segment(aes(x = 49.5, xend = 49.5, y = 0, yend = 85), color = "darkgreen", linetype = "dashed") +
  ylab("Frequency") +
  xlab("Chi Squared (for group * remaining time interaction)") +
  ...
  theme_minimal()
print(gg)
ggsave(filename = here("output", "X2_hist_plot.pdf"), plot = gg, width = 7, height = 5)
...
```{r}
em <- emtrends(glm1.sl.2, pairwise ~ group , var = "remaining_time_reversed", adjust=adjustment_method)
em
...
g <- ggplot(aes(y = F1, x = remaining_time_reversed), data = df2) +
