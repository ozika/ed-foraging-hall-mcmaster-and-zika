...
df_group <- data %>% 
  group_by(id, group, rem_time_q) %>% 
  summarise(across(c(logRT, remaining_time), mean), .groups = "drop")

g1 <- ggplot(df_group, aes(y = logRT, x = remaining_time, color = group)) +
  geom_point(alpha = alpha) +
  geom_smooth(method = 'lm', se = TRUE) +
  scale_x_reverse() +
  scale_color_manual(
    values = c("maroon4", "dodgerblue"),
    labels = c("sED", "HC"),
    name = "Group"
  ) +
  xlab("Remaining Time") +
...
print(g1)
ggsave(filename = here("output", "plot_group.pdf"), plot = g1, width = wi, height = he)
...
g2 <- ggplot(df_f1, aes(y = logRT, x = remaining_time, color = F1.ms)) +
  geom_point(alpha = alpha) +
  geom_smooth(method = 'lm', se = TRUE) +
  scale_x_reverse() +
  scale_color_manual(values = c("dodgerblue", "maroon4")) +
  xlab("Remaining Time") +
  ggtitle("Split by F1")
print(g2)
ggsave(filename = here("output", "plot_F1.pdf"), plot = g2, width = wi, height = he)
...
g3 <- ggplot(df_f2, aes(y = logRT, x = remaining_time, color = F2.ms)) +
  geom_point(alpha = alpha) +
  geom_smooth(method = 'lm', se = TRUE) +
  scale_x_reverse() +
  scale_color_manual(values = c("dodgerblue", "maroon4")) +
  ...
  ggtitle("Split by F2")
print(g3)
ggsave(filename = here("output", "plot_F2.pdf"), plot = g3, width = wi, height = he)
...
g4 <- ggplot(df_f3, aes(y = logRT, x = remaining_time, color = F3.ms)) +
  geom_point(alpha = alpha) +
  geom_smooth(method = 'lm', se = TRUE) +
  scale_x_reverse() +
  scale_color_manual(values = c("dodgerblue", "maroon4")) +
  ...
  ggtitle("Split by F3")
print(g4)
ggsave(filename = here("output", "plot_F3.pdf"), plot = g4, width = wi, height = he)
```
