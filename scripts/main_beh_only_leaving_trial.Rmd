...
```{r}
data <- read.csv(here::here( 'anon-data', 'task_data', 'leaving_time_data.csv'))
...
ggsave(
  ...
  units = c("in"),
  dpi = 300)
write.csv(df_long, here::here("output", "data_patch_time.csv"))
```
...
performance::compare_performance(m2,glm2, m2.sl, glm2.sl, glm2.sl.2, glm2.sl.trvl, glm2.sl.dec)
```
...
ggsave(
  ...
  dpi = 300)
  #theme(axis.text.x = element_text(angle = 45, hjust = 1))
write.csv(df_long, here::here("output", "data_reward.csv"))
```
...
pretty_table <- df2 %>%
  ...
  save_kable(here::here("output", "rewards.png"))
